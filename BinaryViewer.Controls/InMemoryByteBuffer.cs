...
namespace BinaryViewer.Controls
{
	...
	/// </summary>
	public class InMemoryByteBuffer : IByteProvider
	{
		/// <summary>
		...
		/// <summary>
		/// Initializes a new instance of the InMemoryByteBuffer class.
		/// </summary>
		/// <param name="data"></param>
		public InMemoryByteBuffer(byte[] data) : this(new List<Byte>(data))
		{
		}
		...
		public InMemoryByteBuffer(List<Byte> bytes)
		{
			_bytes = bytes;
		}
		...
		{ 
			int clampedIndex = (int)Math.Max(0, index);
			int clampedLength = (int)Math.Min(Length - clampedIndex, length);
			_bytes.RemoveRange(clampedIndex, clampedLength);
